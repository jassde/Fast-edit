...
- **`project.rs`** — save/load `.vtproj.json` (schema version 1: `filePath`, `duration`, `playheadPosition`, `segments[]`). `default_save_dir` resolves `Documents\Fast-edit\saves`. `load_project` validates the version field and returns a `Project` struct the frontend rehydrates (`App.tsx` restores the playhead via a `pendingSeekRef` after mpv finishes loading the file).
