...
/// Persists a new yt-dlp.exe path and updates the running state.
#[tauri::command]
...
pub fn fetch_formats(
    ...
    // Collect heights from streams that carry video
    ...
    // If no video heights matched, add a generic fallback
    if result.len() == 1 {
        result.insert(
            0,
            serde_json::json!({
...
pub async fn download_video(
    ...
    // Parse stderr for progress lines on a background thread
...
/// Deletes all files inside the Temp folder (but keeps the folder itself).
#[tauri::command]
